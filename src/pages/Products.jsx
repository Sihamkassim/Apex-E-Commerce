...
const Products = () => {
  ...
  return (
    <div className="grid grid-cols sm:grid-cols-2 lg:grid-cols-4 py-3 px-80 gap-3 w-full">
      {data.map((ListOfProduct, i) => (
        <div
          key={i}
        ...
        >
          {Products.price < 15.00 || 25.00? 
            <span className="absolute top-2 right-2 bg-yellow-400 text-white text-xs font-semibold px-2 py-1 rounded">
              Recommended
            </span> :''
          }
          <img
            src={ListOfProduct.images}
            alt={ListOfProduct.name}
            className="w-full h-48 object-contain mb-3 rounded-md bg-gray-50"
          />
          <h2 className="text-lg font-semibold line-clamp-1">
            {ListOfProduct.name}
          </h2>
          <img src={ListOfProduct.review} alt="rating" className="h-4 mb-1" />
          <p className="text-gray-500 text-xs mb-2">{ListOfProduct.stock}</p>
          <p className="text-black font-bold mt-2">{ListOfProduct.price}</p>
          <Link to={`/products/${ListOfProduct.name}`}>
